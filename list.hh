...
template <class Type> class List_item { // List_item<Type>: item in linked list
...
template <class Type> List_item<Type>::List_item(Type *x) { // constructor
...
template <class Type> class List { // List<Type>: linked list or stack of items
public:
...
template <class Type> Type *List<Type>::pop() { // pop item off stack
  List_item<Type> *p = firstp;
  if (!p)
    return 0; // stack empty, can't pop
  firstp = p->next();
  if (!firstp)
    lastp = 0;
  Type *x = p->obj;
...
template <class Type> int List<Type>::length() const { // return length of list
  List_item<Type> *p;
  int count;
...
template <class Type> void List<Type>::free_items() {
...
template <class Type> void List<Type>::free_all() {
  Type *x;
  do {
    x = pop();
    if (x)
...
template <class Type> void List<Type>::copy(const List<Type> &l) {
...
template <class Type> void List<Type>::dup() { // duplicate top item
  if (!firstp) {
    std::cerr << "ERROR: attempt to dup() first item in empty list"
              << std::endl;
    exit(1);
  }
